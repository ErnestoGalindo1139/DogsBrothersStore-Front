export const ProductsCard = ({ producto }) => {
    ...
    const { image, name,  availableColors } = producto;

    return (
        <li className="inline-flex w-64 flex-col text-center lg:w-auto">
            <div className="group relative">
                <div className="aspect-h-1 aspect-w-1 w-full overflow-hidden rounded-md bg-gray-200">
                    <img src={ image } alt={ name } className="h-full object-cover object-center group-hover:opacity-75" />
                </div>
                <div className="mt-6">
            ...
            <ul role="list" className="mt-auto flex items-center justify-center space-x-3 pt-6">
                {
                    availableColors.map(availableColor => (
                        <li key={availableColor.colorCode} className="h-4 w-4 rounded-full border border-black border-opacity-10" style={{backgroundColor: availableColor.colorCode}}>
